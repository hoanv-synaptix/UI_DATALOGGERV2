...
static void ensure_overlays(view_factory_t *ui)
{
...
void ui_input_policy_init_constraints(view_factory_t *ui)
{
    ...
    size_t ta_count = view_factory_textarea_count();

    for (size_t id = 0; id < ta_count; id++) {
        lv_obj_t *ta_obj = view_factory_get_textarea(ui, (ui_textarea_id_t)id);
        
        if (ta_obj && lv_obj_is_valid(ta_obj)) {
            
            // CHÚ Ý: Thay LV_STATE_DEFAULT bằng LV_STATE_FOCUSED
            // Con trỏ sẽ CHỈ hiển thị và dày lên khi ô đó được Focus
            lv_obj_set_style_width(ta_obj, 3, LV_PART_CURSOR | LV_STATE_FOCUSED);
            lv_obj_set_style_bg_color(ta_obj, lv_color_hex(0x00A2E8), LV_PART_CURSOR | LV_STATE_FOCUSED);
            lv_obj_set_style_bg_opa(ta_obj, LV_OPA_COVER, LV_PART_CURSOR | LV_STATE_FOCUSED);
            lv_obj_set_style_anim_time(ta_obj, 400, LV_PART_CURSOR | LV_STATE_FOCUSED);

            // Viền ô nhập liệu cũng chỉ dày lên khi được Focus
            lv_obj_set_style_border_width(ta_obj, 2, LV_PART_MAIN | LV_STATE_FOCUSED);
            lv_obj_set_style_border_color(ta_obj, lv_color_hex(0x00A2E8), LV_PART_MAIN | LV_STATE_FOCUSED);
            
            // Đảm bảo ở trạng thái bình thường (Default), con trỏ không chiếm diện tích hiển thị
            lv_obj_set_style_width(ta_obj, 0, LV_PART_CURSOR | LV_STATE_DEFAULT);
        }
    }
}
...
bool ui_input_policy_is_enabled(view_factory_t *ui, lv_obj_t *input_obj)
{
    ...
    view_factory_get_network_refs(ui, &refs);
    group = view_factory_classify_input(ui, input_obj);
...
void ui_input_policy_apply(view_factory_t *ui, const ui_runtime_state_t *state)
{
    ...
    ensure_overlays(ui);
    view_factory_get_network_refs(ui, &refs);
    wifi_ip_ta = view_factory_get_textarea(ui, UI_TEXTAREA_WIFI_IP);
    wifi_subnet_ta = view_factory_get_textarea(ui, UI_TEXTAREA_WIFI_SUBNET);
    wifi_gateway_ta = view_factory_get_textarea(ui, UI_TEXTAREA_WIFI_GATEWAY);
    eth_ip_ta = view_factory_get_textarea(ui, UI_TEXTAREA_ETHERNET_IP);
    eth_subnet_ta = view_factory_get_textarea(ui, UI_TEXTAREA_ETHERNET_SUBNET);
    eth_gateway_ta = view_factory_get_textarea(ui, UI_TEXTAREA_ETHERNET_GATEWAY);
