...
/**
 * @brief Hàm trợ giúp để Gắn (Bind) sự kiện vào Widget một cách an toàn.
 * Xóa bỏ sự kiện cũ (nếu có) trước khi gắn mới để tránh lỗi Trigger 2 lần.
 */
...
/**
 * @brief Hàm trợ giúp để Bắn (Dispatch) một Action không kèm dữ liệu (value = 0)
 * tới State Machine trung tâm.
 */
static void dispatch_simple(lv_event_t *e, ui_action_t action)
{
    view_factory_t *ui = lv_event_get_user_data(e);
...
static void btn_mqtt_apply_clicked_cb(lv_event_t *e) { dispatch_simple(e, UI_ACTION_SUBMIT_MQTT_CONFIG); }
static void btn_modbus_apply_clicked_cb(lv_event_t *e) { dispatch_simple(e, UI_ACTION_SUBMIT_MODBUS_CONFIG); }

/**
 * @brief Xử lý nút xóa lịch sử sự kiện (Event Logs)
 * Duyệt qua tất cả các phần tử trong danh sách và xóa chúng, giữ lại Tiêu đề và Nút bấm.
 */
static void btn_clear_history_clicked_cb(lv_event_t *e)
{
    ...
    if (!ui) return;
    view_factory_get_logs_refs(ui, &refs);
    if (!refs.logs_container || !lv_obj_is_valid(refs.logs_container)) return;

    // Quét ngược từ cuối danh sách để xóa không bị lỗi Index
...
void ui_events_navigation_init(view_factory_t *ui)
{
    ...
    // Thu thập tham chiếu đến các đối tượng giao diện
    view_factory_get_shell_refs(ui, &shell_refs);
    view_factory_get_logs_refs(ui, &logs_refs);
    ...
    // Gắn (Bind) sự kiện Nhấp chuột ngắn (Short Click) cho từng nút bấm
    ...
    bind_cb(mqtt_refs.btn_mqtt_apply, btn_mqtt_apply_clicked_cb, LV_EVENT_SHORT_CLICKED, ui);
    bind_cb(modbus_refs.btn_modbus_apply, btn_modbus_apply_clicked_cb, LV_EVENT_SHORT_CLICKED, ui);

    // Gắn (Bind) sự kiện Thay đổi giá trị (Value Changed) cho các Dropdown
